...
static inline unsigned int FindNextChar(unsigned int start, const char *str,
                                        unsigned int length, char token);
static inline unsigned int ParseOBJIndexValue(const std::string &token,
                                              unsigned int start,
                                              unsigned int end);
static inline float ParseOBJFloatValue(const std::string &token,
  ...
  for (unsigned int i = 0; i < numIndices; i++) {
    ...
    if (it == normalModelIndexMap.end()) {
      ...
      normalModelIndexMap.insert(
          std::pair<OBJIndex, unsigned int>(*currentIndex, normalModelIndex));
    ...
    unsigned int previousVertexLocation =
...
unsigned int
OBJModel::FindLastVertexIndex(const std::vector<OBJIndex *> &indexLookup,
  ...
  while (current != previous) {
    ...
    if (testIndex->vertexIndex == currentIndex->vertexIndex) {
      ...
      for (unsigned int i = countStart; i < indexLookup.size() - countStart;
           i++) {
        OBJIndex *possibleIndex = indexLookup[current + i];
...
OBJIndex OBJModel::ParseOBJIndex(const std::string &token, bool *hasUVs,
                                 bool *hasNormals) {
  unsigned int tokenLength = token.length();
  const char *tokenString = token.c_str();

  unsigned int vertIndexStart = 0;
  unsigned int vertIndexEnd =
